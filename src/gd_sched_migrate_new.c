...
void* proc_main(void* arg){
    ...
    while(running && (period < nperiods)){
        ...
        for (int cur = 0; cur<proc_nthreads;cur++) {
            ...
            int lim_off = floor(avail_time/(sub_fft_time*num_ants));
            ...
            if (noff>0) {
                ...
                migrate_avail[cur].source_bs_id = bs_id;
            }
            cur_start_id +=noff;
        ...
        if (rem_time > 50){
            ...
            while( timespec_to_usec(&t_now) <=  timespec_to_usec(&t_next)-50 ) {
                ...
                clock_gettime(CLOCK_MONOTONIC, &t_now);
                rem_time = 0;//timespec_to_usec(&t_next)  - timespec_to_usec(&t_now);
                // if (rem_time>50) {
                //     state[id] = timespec_to_usec(&t_next);
                // }
...
int main(int argc, char** argv){
    ...
    for (i=0; i<proc_nthreads; i++){
        ...
        migrate_avail[i].start_id = 0;
        migrate_avail[i].source_bs_id = 0;

...
static int subframe_avail=0;
static int running;
static int debug_trans = 1;
static int mcs;
static int var;

gd_rng_buff_t buff[MAX_LEN];
...
void* trans_main(void* arg){
    ...
    struct timespec t_next, t_deadline, trans_start, trans_end, t_temp, t_now, t_add;
    struct timespec trans_dur = usec_to_timespec(trans_dur_usec);

    t_add = usec_to_timespec(2000 - trans_dur_usec);
    ...
    while(running && (period < nperiods)){
        ...
        if (subframe_avail == MAX_LEN){

            // log_error("Buffer Full. Dropping %d bs\n", bs_id );
        }else{

            buff[subframe_avail].mcs = curr_mcs;
            buff[subframe_avail].bs_id = bs_id;
            clock_gettime(CLOCK_MONOTONIC, &t_temp);
            t_temp = timespec_add(&t_temp, &t_add);
            buff[subframe_avail].t_deadline = t_temp;
            subframe_avail ++;
            // printf("Added subframe transport %d. subframe_avail = %d\n", id, subframe_avail);
        }
...
void* proc_main(void* arg){
    ...
    log_notice("Starting proc thread %d nperiods %d", id, nperiods);
    ...
    while(running && (period < nperiods)){
        ...
        if (timespec_lower(&t_now, &t_deadline)){
...
int main(int argc, char** argv){
    ...
    char c;
    while ((c = getopt (argc, argv, "h::M:A:L:s:d:p:S:e:D:Z:N:m:R:C:")) != -1) {
        switch (c) {
            case 'M':
              num_bss = atoi(optarg);
              break;
    ...
    for(i= 0; i < trans_nthreads; i++){
        ...
        // sprintf(tmp_str, "../log_global/exp%d_samp%d_trans%d_prior%d_sched%s_nbss%d_nants%d_ncores%d_Lmax%d_mcs%d_delay%d.log",
        //     var, num_samples, i, priority, tmp_str_a, num_bss, num_ants, proc_nthreads, lmax, mcs, trans_dur_usec);
    ...
    for(i= 0; i < proc_nthreads; i++){
        ...
        sprintf(tmp_str, "../log_global/exp%d_samp%d_proc%d_prior%d_sched%s_nbss%d_nants%d_ncores%d_Lmax%d_snr%d_mcs%d_delay%d.log",
            var, num_samples, i, priority,tmp_str_a, num_bss, num_ants, proc_nthreads, lmax, snr, mcs, trans_dur_usec);

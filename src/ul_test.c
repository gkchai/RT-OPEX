...
int main(int argc, char **argv){
  ...
  tparam.sched_priority = sched_get_priority_max(SCHED_FIFO);
   ...
   short* iqr = (short*) malloc(1*2*307200*sizeof(short)); //1=no_of_frame/1000, 2=BW/5MHz
   short* iqi = (short*) malloc(1*2*307200*sizeof(short));
  ...
  sprintf(filename_iq, "/mnt/hd3/gkchai/ul_test/test_iq_mcs=%d_nrb=%d_subf=%d_ch=%d_rx=%d.dat", mcs, nrb, subf, ch, rx);
  printf("Reading ... %s\n", filename_iq);
 ...
 if (PHY_vars_eNB->ulsch_eNB[0] != NULL)
  ...
  for (ch_realization=0; ch_realization<n_ch_rlz; ch_realization++) {
    ...
    for (SNR=snr0; SNR<snr1; SNR+=input_snr_step) {
      ...
      sprintf(bler_fname,"/home/gkchai/gkchai/fall15/garud/log/ul_test/ULbler_mcs%d_snr%f_nrb%d_subf%d_ch%d_rx=%d_miter=%d.csv",mcs, SNR, nb_rb, subframe, chMod, n_rx, max_turbo_iterations);
      bler_fd = fopen(bler_fname,"w");
      fprintf(bler_fd,"#SNR;mcs;nb_rb;TBS;rate;errors[0];trials[0];errors[1];trials[1];errors[2];trials[2];errors[3];trials[3]\n");

      sprintf(time_meas_fname,"/home/gkchai/gkchai/fall15/garud/log/ul_test/time_mcs=%d_snr%f_nrb=%d_subf=%d_ch=%d_rx=%d_miter=%d.csv", mcs, SNR, nb_rb, subframe, chMod, n_rx, max_turbo_iterations);
      ...
      double temp[2*PHY_vars_eNB->lte_frame_parms.samples_per_tti];
      ...
      for (trials = 0; trials<n_frames; trials++) {
        ...
        while (round < 4) {
          ...
          int gg;
          for (gg=0; gg<PHY_vars_eNB->lte_frame_parms.samples_per_tti; gg++){

              temp[2*gg] =  iqr[gg  + round*PHY_vars_eNB->lte_frame_parms.samples_per_tti];
              temp[2*gg + 1] = iqi[gg  + round*PHY_vars_eNB->lte_frame_parms.samples_per_tti];
          }


          tx_lev = signal_energy(&temp, PHY_vars_eNB->lte_frame_parms.samples_per_tti);
          ...
          // printf("trails = %d, round=%d, tx_lev = %d (%d.%d dB,%f), gain %f\n", trials, round, tx_lev,tx_lev_dB/10,tx_lev_dB,10*log10((double)tx_lev),10*log10(tx_gain));
          ...
          for (i=0; i<PHY_vars_eNB->lte_frame_parms.samples_per_tti; i++) {
            for (aa=0; aa<1; aa++) {
              s_re[aa][i] = (double)iqr[i  + round*PHY_vars_eNB->lte_frame_parms.samples_per_tti];
              s_im[aa][i] = (double)iqi[i  + round*PHY_vars_eNB->lte_frame_parms.samples_per_tti];
            }
          }
          ...
          if (awgn_flag == 0) {
            ...
            for(i=0; i< PHY_vars_eNB->lte_frame_parms.samples_per_tti; i++){
                ...
                ((short*) &PHY_vars_eNB->lte_eNB_common_vars.rxdata[0][aa][PHY_vars_eNB->lte_frame_parms.samples_per_tti*subframe])[2*i+1] = (short) ((tx_gain*r_im[aa][i]) + (iqim*1*r_re[aa][i]) + sqrt(sigma2/2)*gaussdouble(0.0,1.0));
            }
          ...
          if (ret <= PHY_vars_eNB->ulsch_eNB[0]->max_turbo_iterations) {
            ...
            turbo_iter[6*n_frames*super_trials + 6*trials + round] = ret;
            harq_rounds[n_frames*super_trials + trials] = round;
            final_err[n_frames*super_trials + trials] = 0;
            ...
            turbo_iter[6*n_frames*super_trials + 6*trials + round] = ret-1;
            harq_rounds[n_frames*super_trials + trials] = round;
            final_err[n_frames*super_trials + trials] = 1;
      ...
      // sorttable(table_rx_dec, time_vector_rx_dec.size);
...

       {
        ...
        sprintf(filename_time,"/home/gkchai/gkchai/fall15/garud/log/ul_test/trial_time_mcs=%d_snr%f_nrb=%d_subf=%d_ch=%d_rx=%d_miter=%d.csv", mcs, SNR, nb_rb, subframe, chMod, n_rx, max_turbo_iterations);
        ...
        for (int super_frames = 0; super_frames<10; super_frames++){
          for(n=0; n<n_frames; n++){
            ...
            for(m=0; m<harq_rounds[n_frames*super_frames + n]+1; m++){

              fprintf(ft, "%f;%f;%d;%d;%d;%d\n", table_rx[count], table_rx_dec[count], super_frames*n_frames + n, turbo_iter[6*n_frames*super_frames + n*6 + m], m, final_err[n_frames*super_frames + n]);
              count--;
             }
          }
        }
